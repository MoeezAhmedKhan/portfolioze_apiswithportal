            ...
            //$chatArray = array();
		   	 ...
		   	 $chatData = [
		   	    ...
		   	    $sql_insert_chat = "UPDATE `group_messages` SET `Data`='$data',`last_message`='$user_massage',
                                        `is_read`='read',`status`='sentmessage', `updated_at`= '$currentDate_time'  WHERE `group_id`='$group_id' AND `sender_id`='$user_id'";
       ...
       else
       {
		    ...
		    if(move_uploaded_file($_FILES['data_uri']['tmp_name'],$data['image']))
		    {
    		   	...
    		   	$chatData = [
    		   	    ...
    		   	    $sql_insert_chat = "UPDATE `group_messages` SET `Data`='$data',`last_message`='$user_massage',
    		   	    `is_read`='read',`status`='sentmessage',`updated_at`='$currentDate_time' WHERE `sender_id`='$user_id' AND `group_id`= '$group_id'";
    		   	    $result_insert_chat = mysqli_query($conn,$sql_insert_chat);
    		   	  //  sendMessage($friend_id,$user_id);		   	    
    		   	  //  // for other user
    		   	  //  $sql_insert_chat2 = "UPDATE `messages` SET `Data`='$data',`last_message`='$user_massage',
