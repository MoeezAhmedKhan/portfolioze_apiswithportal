...
$sql = "SELECT `id`, COUNT(`company_id`) as countz FROM `jobs` WHERE `company_id` = '$company_id' GROUP BY `created_at`";
$exec_sql = mysqli_query($conn,$sql);

   
        $data_array = array();
        
        $sql2 = "SELECT COUNT(`id`) as jobs_posted FROM `jobs` WHERE `company_id` = '$company_id' ";
        $exec_sql2 = mysqli_query($conn,$sql2);
        $rowz = mysqli_fetch_array($exec_sql2);
        
        
        
        $sql4 = "SELECT COUNT(`ja_id`) as interview_count FROM `job_applications_log` WHERE `company_id` = '$company_id' AND `status` = 'interviewed'";
        $exec_sql4 = mysqli_query($conn,$sql4);
        $row4 = mysqli_fetch_array($exec_sql4);
        
        
        $sqli = "SELECT COUNT(`ja_id`) as jobs_posted FROM `job_applications_log` WHERE `company_id` = '$company_id' GROUP BY `created_at`";
