...
include('../connection.php');
  ...
  $sql= "SELECT `frd_id`, `friendA_id`, `friendB_id`, `created_at` FROM `friends` WHERE  `friendA_id` = '$userid' OR `friendB_id` = '$userid'";
  ...
  if(mysqli_num_rows($exec_sql) > 0){
      ...
      while($row = mysqli_fetch_array($exec_sql)){
          ...
          $friend_A = $row['friendA_id'];
          $friend_B = $row['friendB_id'];
          
          $sql2 = "SELECT `id`, `role`, `full_name`, `email`, `phone_number`, `password`, `img`, `status`, `notification_token`, 
          `social_id`, `isActive`,`created_at`, `updated_at` FROM `users` WHERE (`id` = '$friend_A' OR `id` = '$friend_B') AND `id` != '$userid' ";
