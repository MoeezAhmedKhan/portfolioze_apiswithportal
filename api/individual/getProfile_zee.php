...
$sql = "SELECT u.id, u.full_name, u.email, u.img, u.cover, t.designation, t.dob , t.about, t.tes_education , t.tes_rewards,t.tes_experience ,
t.tes_project , t.portfolio_links,t.facebook, t.youtube, t.twitter, t.skype FROM users u INNER JOIN `testimonials` t  ON t.user_id = u.id WHERE `id` = '$user_id'";
    ...
    if(mysqli_num_rows($exec_sql) > 0){
        ...
        while($rows = mysqli_fetch_array($exec_sql)){
            ...
            $temp = [
                    ...
                    "tes_education"=>json_decode($rows['tes_education']) != null ? json_decode($rows['tes_education']) : [],
                    "tes_experience"=>json_decode($rows['tes_experience']) != null ? json_decode($rows['tes_experience']) : [],
        ...
        $data = ["status"=>false,
            "Response_code"=>203,
            "profile_data"=>[],
            "Message"=>"There is no profile to show."];
        echo json_encode($data);
    }
