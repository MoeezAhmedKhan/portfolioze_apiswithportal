   ...
   if(mysqli_num_rows($result_check_chat) > 0)
   {
     ...
     else
     {
          if($data_type=="text")
          {
		   	    ...
		   	    $sql_insert_chat = "INSERT INTO `messages`(`sender_id`, `reciever_id`, `Data`,`last_message`, `is_read`, `status`,`updated_at`) VALUES 
		   	    ($user_id,$friend_id,'$data','$user_massage','read','sentmessage','$currentDate_time')";
		   	    $result_insert_chat = mysqli_query($conn,$sql_insert_chat);
                sendMessage($friend_id,$user_id);
		   	    
		   	    
		   	    $sql_insert_chat2 = "INSERT INTO `messages`(`sender_id`, `reciever_id`, `Data` , `last_message`, `is_read`, `status`,`updated_at`) VALUES 
		   	    ($friend_id,$user_id,'$data', '$user_massage','unread','newmessage','$currentDate_time')";
      	    ...
      	    else
      	    {
    		    ...
    		    if(move_uploaded_file($_FILES['data_uri']['tmp_name'],$data['image']))
    		    {
    		   	    ...
    		   	    `last_message`, `is_read`, `status`,`updated_at`) VALUES 
    		   	    ($user_id,$friend_id,'$data','$user_massage','read','sendmessage','$currentDate_time')";
