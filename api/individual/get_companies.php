...
$sql = "SELECT u.id,  u.full_name, u.img FROM `users` u WHERE  u.role = 'Company' LIMIT 5";
$exec_sql = mysqli_query($conn,$sql);

 $user_membership = "SELECT `id`, `membership_id`, `user_id`, `expiration_date`, `status`, `paid_amount`, `created_at` 
 FROM `tbl_membership_log` WHERE `user_id`= $userid AND `status` = 'Active'";
